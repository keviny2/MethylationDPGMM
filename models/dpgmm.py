...
class DPGMM(object):
    ...
    def __init__(self,
        ...
        self.burnin_epsilon = np.log(2)

        self.inference_epsilon = np.log(2)
    ...
    def calculate_log_integral_matrix_c_j(self):
        ...
        log_integral_matrix = np.zeros(shape=(self.K, self.M))

        for c in range(self.K):

            for j in range(self.M):

                log_integral_matrix[c, j] = self.calculate_log_integral_term_c_j(c, j)

        return log_integral_matrix

    def calculate_log_integral_term_c_j(self, c, j):

        data_points = self.cluster_to_index[c]

        N_c = len(data_points)

        # mu_cj, kappa_cj, alpha_cj, beta_cj = self.calculate_posterior_parameters_given_priors(j, data_points,
        ...
        mu_cj, kappa_cj, alpha_cj, beta_cj = self.calculate_posterior_parameters_given_priors_using_sufficient_statistics(self.cluster_counts[c],
                                                                                                                          self.sum_b[c, j],
                                                                                                                          self.sum_squared_b[c, j],
                                                                                                                          self.base_distribution['mean'][j],
                                                                                                                          self.base_distribution['precision'][j],
                                                                                                                          self.base_distribution['alpha'][j],
                                                                                                                          self.base_distribution['beta'][j])

        term0 = -(N_c/2) * np.log(2*np.pi)

        term1 = - self.calculate_normal_gamma_normalization_constant(self.base_distribution['precision'][j], self.base_distribution['alpha'][j], self.base_distribution['beta'][j])

        term2 = self.calculate_normal_gamma_normalization_constant(kappa_cj, alpha_cj, beta_cj)
    ...
    def calculate_log_integral_matrix_given_sufficient_statistics(self, K, cluster_counts, sum_b, sum_squared_b):
        ...
        log_integral_matrix = np.zeros(shape=(K, self.M), dtype=np.float64)

        for c in range(K):

            for j in range(self.M):

                log_integral_matrix[c, j] = self.calculate_log_integral_term_given_sufficient_statistics(j, cluster_counts[c], sum_b[c, j], sum_squared_b[c, j])
    ...
    def calculate_log_integral_term_given_sufficient_statistics(self, j, N, sum_data, sum_squared_data):
        ...
        mu_n, kappa_n, alpha_n, beta_n = self.calculate_posterior_parameters_given_priors_using_sufficient_statistics(N,
                                                                                                                      sum_data,
                                                                                                                      sum_squared_data,
                                                                                                                      self.base_distribution['mean'][j],
                                                                                                                      self.base_distribution['precision'][j],
        ...
        term2 = self.calculate_normal_gamma_normalization_constant(kappa_n, alpha_n, beta_n)

        return term0 + term1 + term2

    def calculate_posterior_parameters_given_priors(self, data_points_idx, mu, kappa, alpha, beta):

        N = len(data_points_idx)

        data = self.shifted_data[data_points_idx]
    ...
    @staticmethod
    def calculate_normal_gamma_normalization_constant(kappa, alpha, beta):

        term0 = loggamma(alpha)
    ...
    def calculate_log_integral_new(self, idx):
        ...
        term2 = self.calculate_normal_gamma_normalization_constant(kappa_nj, alpha_nj, beta_nj)
    ...
    def calculate_log_integral_existing(self, idx, data_points):
        ...
        term1 = -self.calculate_normal_gamma_normalization_constant(kappa_0, alpha_0, beta_0)
    ...
    def calculate_new_proposals(self, acceptance_rate, current_proposals, burnin):
        ...
        if burnin:
            ...
            # TODO: decay in burn in so possibly dont jump to much
        ...
        low_indices = acceptance_rate < 0.7
